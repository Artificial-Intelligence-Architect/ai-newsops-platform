...
Une démo complète qui fonctionne SANS Docker et SANS modèle pré-entraîné.
Utilise le modèle DistilBERT pré-entraîné de HuggingFace.

Usage:
    python demo_standalone.py

Temps d'exécution: ~2-3 minutes (download + demo)
"""
...
def load_model():
    ...
    print_info("Using: distilbert-base-uncased (HuggingFace)")
    print_info("This is a pre-trained model (not fine-tuned on your data)")
    print_info("For production, use: models/distilbert/best_model/")
    
    try:
        model_name = "distilbert-base-uncased"
        print(f"\n{Colors.CYAN}Downloading model...{Colors.END}")
