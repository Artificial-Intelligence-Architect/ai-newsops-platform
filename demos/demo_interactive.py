...
Test the model with your own headlines and descriptions.
...
# Label mapping (matches models/distilbert/label_mapping.json)
LABEL_MAPPING = {
    0: "arts_culture",
    1: "business",
    2: "crime",
...
def load_model():
    """Load the fine-tuned model if available, otherwise fall back to the generic one"""
    ...
    is_finetuned = (
        ...
        and os.path.getsize(safetensors_path) >= 100_000_000
    )

    model_path = finetuned_path if is_finetuned else "distilbert-base-uncased"
    ...
    if is_finetuned:
        print(f"{Colors.GREEN}✅ Fine-tuned model ready!{Colors.END}\n")
    else:
        print(f"{Colors.YELLOW}⚠️  Fine-tuned model not found, using generic HuggingFace model{Colors.END}")
        print(f"{Colors.YELLOW}   Predictions will not be meaningful (untrained classification head){Colors.END}\n")
